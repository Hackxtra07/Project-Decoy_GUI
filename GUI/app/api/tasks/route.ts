...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'cancel') {
      ...
      await db.prepare("UPDATE commands SET status = 'cancelled', is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?").all([taskId])
      ...
      `).all([cancelId, clientId, JSON.stringify({ target_id: taskId })])
