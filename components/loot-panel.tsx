...
import { Folder, File, ArrowLeft, Download, MousePointer2, Clock, HardDrive, Search, Eye, FileText, Image as ImageIcon } from 'lucide-react'
        ...
        <Card className="fixed inset-4 md:inset-10 z-50 flex flex-col bg-background/95 backdrop-blur-md shadow-2xl border-sidebar-border overflow-hidden animate-in zoom-in duration-200">
           ...
           <CardContent className="flex-1 overflow-auto p-0 bg-black/40">
                {fileContent === 'IMAGE_DISPLAY' ? (
                  ...
                  <pre className="p-6 font-mono text-sm leading-relaxed text-blue-100">
                    {fileContent}
                  </pre>
                ) : (
                  <div className="w-full h-full flex flex-col items-center justify-center gap-4 text-muted-foreground">
                    <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-blue-500"></div>
                    <p>Decrypting captured resource...</p>
                  </div>
                )}
           </CardContent>
        </Card>
